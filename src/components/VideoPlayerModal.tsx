...
export default function VideoPlayerModal({ file, accessToken, onClose }: VideoPlayerModalProps) {
  ...
  // Route the video through your own Vercel backend proxy to bypass office firewalls
  const proxyStreamUrl = `/api/stream?fileId=${file.id}&token=${accessToken}`;
            ...
            <div className="flex items-center gap-3 truncate">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 flex items-center gap-1.5 mt-0.5">
                  <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" /> Firewall-Bypassed Vercel Proxy Stream Active
                </p>
              </div>
            </div>
            <button
          ...
          <div className="relative bg-black aspect-video flex items-center justify-center overflow-hidden">
            {!isVideoLoaded && (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-950 z-10 gap-3">
                ...
                <p className="text-sm font-semibold text-slate-400 flex items-center gap-2">
                  <Sparkles className="w-4 h-4 text-brand-400 animate-pulse" /> Piping secure stream through Vercel...
                </p>
              </div>
            )}
            ...
            <video
              ...
              onError={() => {
                ...
                alert("Failed to load stream via proxy. Check console logs.");
              }}
            />
          </div>
        </motion.div>
